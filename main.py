#Juego Tic tac (sin aplicar IA)
#author Agesta Mariano
...
def EnterMove(board):
	ok = False	# suposición falsa - se necesita para entrar al bucle
	while not ok:
		move = input("Ingresa tu movimiento: ") 
		ok = len(move) == 1 and move >= '1' and move <= '9' # ¿es valido lo que ingreso el usuario?
		if not ok:
			print("Movimiento erróneo, ingrésalo nuevamente") # no, no lo es. ingresalo nuevamente.
			continue
		move = int(move) - 1 	# numero de la celda, del 0 al 8
		fila = move // 3 	# fila de la celda
		columna = move % 3		# columna de la celda
		sign = board[fila][columna]	# marca el cuadro elegido
		ok = sign not in ['O','X'] 
		if not ok:	# esta ocupado, ingresa una posición nuevamente
			print("¡Campo ocupado, ingresa nuevamente!")
			continue
	board[fila][columna] = 'O' 	# colocar '0' al cuadro seleccionado

def MakeListOfFreeFields(board):
	libre = []	# la lista esta vacia inicialmente
	for fila in range(3): # itera a traves de las filas
		for columna in range(3): # itera a través de las columnas
			if board[fila][columna] not in ['O','X']: # ¿Está la celda libre?
